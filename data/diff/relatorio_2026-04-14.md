# Diferença da base PC 32 - 2026-04-14

- Snapshot atual: `2026-04-14`
- Snapshot anterior: `2026-04-13`
- Total anterior: **1579**
- Total atual: **1579**

## Resumo

- Entraram na base: **0**
- Saíram da base: **0**
- Registros com alguma alteração: **73**
- Registros com mudança de dados de origem: **73**
- Registros com mudança apenas em campos derivados/status: **0**

## Mudanças por status

- `status_suspensiva`: **2**
- `status_pub_licitacao`: **8**
- `status_homolog_licitacao`: **8**
- `status_inicio_obra`: **16**
- `status_regra_casa_civil`: **1**
- `urgencia_suspensiva`: **5**
- `fase_atual`: **28**

## Leitura recomendada

- `categoria_alteracao = dados_e_derivados`: mudou dado de origem e isso também repercutiu em campos calculados.
- `categoria_alteracao = dados`: mudou apenas dado de origem, sem alterar status calculado.
- `categoria_alteracao = derivados_tempo`: o dado-base ficou igual; a alteração veio só de regra derivada/data corrente.
